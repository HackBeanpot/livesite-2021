export const scheduleExtractor = (data) => {
  let sortedData = data.sort((a, b) => new Date(a.fields.startTime) - new Date(b.fields.startTime));
  let schedule = {};
...
export const eventsExtractor = (attributes) => {
  ...
  const time = attributes.endTime ? `${startTime}-${timeExtractor(attributes.endTime)}` : startTime;
  let event = {
    "time": time,
