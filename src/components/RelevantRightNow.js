...
const RelevantCard = ({ title, type, body, buttonURL, buttonText, displayStartTime, displayEndTime }) => (
    <Col xs="12" md="6" lg="4">
        <Card className={`relevant ${type === "Event" ? "blue-card" : "orange-card"}`}>
            <Card.Body>
                <Card.Title>{title}</Card.Title>
                <Card.Subtitle>{timeText(displayStartTime, displayEndTime)}</Card.Subtitle>
                <Card.Text>
                    {body}
                </Card.Text>
                <a className="btn" href={buttonURL} role="button">{buttonText}</a>
            </Card.Body>
        </Card>
    </Col>
);
...
function timeText (start, end) {
    ...
    return start == null ? `Complete by ${endString}` : `${startString} - ${endString}`;
}
...
const RelevantRightNow = () => {
    ...
    return (
        <Container className="mt-5">
            <Row>
                <Col>
                    <h1 id="relevant" className="font-weight-bold">Relevant Right Now</h1>
                </Col>
            </Row>
